          ...
          <div class="items">
            ${filtered.length ? filtered.map((m) => {
              ...
              return `<button class="item${m.available ? '' : ' out'}" data-mid="${m.id}" ${m.available ? '' : `title="${retail ? 'Out of stock / unavailable' : '86 — unavailable'}"`}>
                ...
                ${retail && m.stock_qty != null ? `<span class="tiny item-stock" style="color:${m.stock_qty <= 0 ? 'var(--red)' : m.stock_qty <= m.stock_min_qty ? 'var(--amber)' : 'var(--dim)'}">${m.stock_mode === 'pour' && m.stock_deduction ? `${Math.floor(m.stock_qty / m.stock_deduction)} servings · ${stockQtyLabel(m.stock_qty, m.stock_unit, m.stock_capacity_ml)}` : stockQtyLabel(m.stock_qty, m.stock_unit, m.stock_capacity_ml)}</span>` : ''}
