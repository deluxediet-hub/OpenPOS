...
- Complimentary issues support full units or measured ml for owner consumption, staff, friends/guests, promotion/tasting or another recorded reason. Owners self-authorize; sellers need a six-digit, one-use, time-limited code generated by an owner, plus a phone/message reference. The login PIN is never shared, failed codes are rate-limited, and reports show both recorder and authorizer. Issues reduce stock (or remain theoretical for weighed kegs), create no payment, leave expected Cash/M-Pesa unchanged, and report lost retail value and inventory cost. Their VAT/eTIMS treatment can differ by purpose, so the future live eTIMS adapter must apply the shop accountant's approved classification rather than silently treating every issue as an ordinary zero-price sale.
