...
- Complimentary issues support full units or measured ml for owner consumption, staff, friends/guests, promotion/tasting or another recorded reason. Owners self-authorize; sellers tick that the owner authorized it and may add a phone/message note. Reports show both the seller who recorded it and the owner account under whose authority it was issued. Issues reduce stock (or remain theoretical for weighed kegs), create no payment, leave expected Cash/M-Pesa unchanged, and report lost retail value and inventory cost. Their VAT/eTIMS treatment can differ by purpose, so the future live eTIMS adapter must apply the shop accountant's approved classification rather than silently treating every issue as an ordinary zero-price sale.
...
- Whole units deduct one stock unit. A compact sales-screen amount dropdown supports Whole, Half, Quarter, ⅛ shot and a visible custom-ml input. Product tiles preview ml and proportional price before selection; price, VAT, cost, availability checks, reporting and stock all use the same stored fraction. Existing products have size backfilled from names such as `250ml` or `750ml` so measured sales do not silently fall back to a whole bottle. Stock calculations retain six-decimal precision and clamp near-zero values, while every UI uses one formatter to show two-decimal bottle equivalents plus exact open-container ml. Stocktake lets staff count sealed units and open ml directly.
...
- Management PDFs inject one unnamed A4 page profile at print time, reset document/root height to auto, retain side margins, and avoid named-page transitions that caused Chromium/Brave to add a blank second sheet. The builder starts with no selections and covers sales, payments, Top 5/all products, category/seller performance, low/full stock, stocktake, expenses, complementaries, reconciliation/till history, product catalogue, deliveries, suppliers, stock movements, loyalty, gift cards, staff and audit. Tables with meaningful monetary or quantity aggregation print a bold total footer; mixed-unit logs print a record count instead of a misleading sum.
