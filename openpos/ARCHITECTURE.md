...
| Final polish & the solo audit | ✅ **Phase 34 done (Days 51–53):** the till is **keyboard-first** — F2 search · F3 customer · F4 tender · F6 pay · F8 hold · F9 quote · ↑↓ select a line · +/− change it · Del remove · F1 lists every one of them on screen (a shortcut nobody can find is not a shortcut). **Empty tables explain themselves** (one `emptyRow()` helper across 56 tables) and **errors arrive as sentences** (`errText()`: permission, stock, subscription and network failures each get their own EN/SW line). **EN/SW is complete**: 403 core strings, zero untranslated, and every `data-i18n` key the pages ask for exists (asserted). **R-C2 solo audit is measured, not promised** — `lib/uiaudit.js` reads the real `pos.html` / `manager.html`, strips the sections a solo shop can never reach and the `data-cap` elements cut at boot, then counts what is left: **0** branch / warehouse / supplier / purchase order / price level / department in a one-till shop (it was 19 before the pass, each one named by file and line). **Receipts belong to the trade**: a 10th module hook (`receipt`) lets each industry add its own lines — the chemist's expiry warning, the wines & spirits age notice — in EN and SW, while the core still never asks which trade it is (R-M1). Responsive to tablet, focus rings, reduced-motion, aria-labels on icon-only buttons, and a real bug fixed: the receipt footer the shop wrote now reaches the paper. 247 tests green (was 240) + 44 UI |
| Pilot release (machine half) | ⚠️ **Phase 35 done in part (Days 54–60):** everything around standing in the shop is built — `lib/pilot.js` provisions the five pilot businesses (general shop · wines & spirits · boutique · pharmacy · multi-branch), each with its own book and sample stock; `observe()` reads each pilot's **own database** and reports trading days, sales, revenue, average basket, offline sales, voids, till variance, stockouts and last activity, so a pilot is judged by evidence rather than by somebody's report; the **friction register** records what confused a cashier, where and how badly (felt twice counts twice) and what fixed it; `goNoGo()` answers go/no-go from five checks — five businesses trading · a full trading week each · the books tie out · no severity-3 friction open · every door in the app opens. `lib/linkaudit.js` walks every `href`, `src` and `fetch()` in the shipped pages against the **298 routes** the server registers: **150 doors, all of them open**. **The human half — deploying to five real Kenyan businesses and watching them trade — cannot be done from here and is not simulated.** 252 tests green (was 247) + 48 UI |
...
- **2026-09-08 (v21)** — **Phase 35 (Days 54–60): pilot release — machine half done, human
  half flagged.** `lib/pilot.js` provisions the five pilot businesses in one call (general
  shop · wines & spirits · boutique · pharmacy · multi-branch), each a real business with its
  own database file, sample catalogue and an owner who can log in, and is idempotent.
  `observe()` opens each pilot's book read-only and reports what actually happened — trading
  days, sales, revenue, average basket, offline sales, suspended and voided sales, till
  variance, lines at zero, last activity — so **a pilot is judged by its own ledger, not by a
  report somebody typed**. The friction register records what went wrong, on which screen, at
  which severity, and what fixed it; the same complaint felt twice is one item counted twice,
  and ranking is by that weight. `goNoGo()` puts the decision on one page: five businesses
  trading · a full trading week each · the books tie out · no severity-3 friction open · every
  door in the app opens — and says **"no-go yet: …"** with the failing checks named, which is
  the honest answer today because no shop has traded a week. `lib/linkaudit.js` is the part of
  "top-10 friction points" a machine can find on its own: it parses every shipped page for
  internal doors and checks them against the 298 registered routes — 150 doors, all open, 65
  built at run-time and left to the tests. Manager gains a **Pilot & go/no-go** card (pilot
  list, friction register with add/fix, the verdict with its reason). **Standing in five real
  shops and watching them trade remains to be done by a human.** 252 tests (was 247) + 48 UI.
- **2026-09-08 (v20)** — **Phase 34 complete (Days 51–53): final UX polish & the solo-mode
  audit.** (a) *Keyboard-first till*: F2/F3/F4/F6/F8/F9, ↑↓/+−/Del and F1 help, with the
  selected cart line highlighted and every shortcut written on the screen. (b) *Empty states
  and plain errors*: `OP.emptyRow()` gives 56 manager tables a real "nothing here yet" with a
  second line, and `OP.errText()` turns permission / stock / subscription / network failures
  into sentences in both languages. (c) *i18n*: EN and SW dictionaries now have **403 keys
  each with zero gaps**, the pages' `data-i18n` keys are asserted to exist, and a
  MutationObserver translates rows drawn after load (they used to keep the old language).
  (d) *R-C2 solo audit*: `lib/uiaudit.js` + `GET /api/solo/audit` read the shipped pages,
  strip gated sections and `data-cap` elements, and count the ERP words left — 19 → **0**,
  each fix a `data-cap` annotation (branch columns, "All branches" filters, supplier fields,
  price levels, the branch-manager dashboard) rather than a deletion, so a chain still sees
  them. (e) *Per-trade receipts*: a 10th collect hook `receipt` lets each module hand the
  core its own receipt lines, printed in the shop's **print language** (`receipt.language`,
  already on the settings screen); the core never names a trade. (f) *Accessibility &
  tablet*: focus-visible rings, aria-labels on icon-only buttons, breakpoints at 1100/900/820/
  600px and `prefers-reduced-motion`. Fixed while wiring it: the receipt footer the shop
  configured was built into the payload but never handed to the printer. 247 tests (was 240)
  + 44 UI steps.
...
- **2026-09-07 (v18)** — **Phase 32 complete (Days 46–48): hardening.** `lib/log.js` (structured JSON lines, request ids, per-route counters; requests log at debug so the console is quiet but the ring buffer and counters always fill); schema versioning in `db.js` (`schema_migrations`, `applyMigrations`, `schemaInfo`) + eleven Phase-32 indexes; `lib/backup.js` (VACUUM INTO snapshots, checksum manifests, AES-sealed copies opened read-only for `integrity_check`, verified restore that keeps the old book, DR drill with RPO/RTO); `/api/admin/{schema,migrate,metrics,backup,backups,restore,dr-drill}`, `/api/health/deep`, `/api/export/*.csv`. Two traps recorded: the book runs in **WAL** mode, so a raw file copy is never safe (VACUUM INTO, or checkpoint then copy); and `node:sqlite` exports **`DatabaseSync`**, not `Database`.
- **2026-09-07 (v17)** — **Phase 31, the part a machine can do: the deliberate breakage matrix** (eight scenarios, above). Three real fixes came out of it: a refund now takes an **amount** (a shop refunds one line, not a whole sale — the route was refunding everything); locking by IP became a **setting** (`lockout.lock_by_ip`), because every till in a Kenyan shop often shares one public address and one person's mistake should not lock out the shop; and the Phase-28 refund reconciliation now recognises `sale/return` as well as `payment/refund`. Left deliberately alone: a partial refund does **not** reopen the sale — returns are modelled on the return, not by netting the payment.
- **2026-09-07 (v16)** — **Phase 29 complete (Days 41–42): owner intelligence.** `lib/intelligence.js` — eight queries, each returning rows + threshold + sentence: cash tied up, branches vs their own history, yesterday's profit, variance root-cause drill, reorder now, discount watch, dead stock, z-score anomalies, plus a digest that can be messaged to the owner. Routes `/api/intelligence/*` and `POST /api/intelligence/digest/send`. Manager **Insights** tab. Two traps worth remembering: `expenses` is dated by `expense_date` (not `spent_at`), and a line discount is posted as `line_discount` (not `discount`).
