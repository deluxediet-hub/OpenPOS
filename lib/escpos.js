...
/* --------------------------------- helpers -------------------------------- */
const money = (cents) => (Number(cents) || 0).toFixed(2);
...
function buildReceipt(r, { paid = true } = {}) {
  ...
  d.bold(false).ln(s.address || '').ln(s.phone || '');
  d.ln(`KRA PIN: ${s.kra_pin || '-'}`).align('left');
  ...
  d.row('Guests', o.people);
  d.row('Served by', (r.waiter || {}).name || '-');
  d.row('Cashier', r.cashier || '-');
  ...
  d.bold(true).row('TOTAL', money(t.total)).bold(false);
  ...
  d.bold(true).big(true).row('AMOUNT DUE', money(t.grand_total)).big(false).bold(false);
  ...
  d.feed(1).hr();
  d.align('center').ln(s.receipt_footer || '');
  {
