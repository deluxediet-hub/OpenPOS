...
class Doc {
  constructor(width = 42, codePage = 'cp437') {
    this.w = width;
    this.chunks = [];
    this.push(...CMD.INIT);
    const pages={cp437:0,cp850:2,cp858:19};
    this.push(ESC,0x74,pages[String(codePage).toLowerCase()]??0);
  }
  ...
  /** Compact item row: keep quantity, product and amount on one line whenever
   * possible; wrap only the product text and keep the amount on the final line. */
  itemRow(left, right) {
    const l = String(left ?? '').replace(/\s+/g, ' ').trim();
    const r = String(right ?? '').trim();
    const finalWidth = Math.max(8, this.w - r.length - 1);
    if (l.length <= finalWidth) return this.row(l, r);

    const target = Math.max(1, l.length - finalWidth);
    let cut = l.indexOf(' ', target);
    if (cut < 1) cut = target;
    const leading = l.slice(0, cut).trim();
    const final = l.slice(cut).trim();
    for (let offset = 0; offset < leading.length; offset += this.w)
      this.ln(leading.slice(offset, offset + this.w));
    return this.row(final, r);
  }
...
/* -------------------------------- receipt --------------------------------- */
function buildReceipt(r, { paid = true, partial = false, reprint = false } = {}) {
  const s = r.settings, o = r.order, t = r.totals || o.totals;
  const W = Number(s.printer_chars) || 42;
  const d = new Doc(W,s.printer_code_page);

  d.align('center').bold(true).big(true).ln(s.business_name).big(false);
  d.bold(false);
  if (s.address && s.receipt_show_address !== '0') d.ln(s.address);
  if (s.phone && s.receipt_show_phone !== '0') d.ln(`Tel: ${s.phone}`);
  if (s.kra_pin && s.receipt_show_kra_pin !== '0') d.ln(`KRA PIN: ${s.kra_pin}`);
  if (s.business_type === 'wines_spirits' && s.licence_number && s.receipt_show_licence !== '0') d.ln(`Licence: ${s.licence_number}`);
  const title=paid?'SALES RECEIPT - PAID':partial?'PART PAYMENT - BALANCE DUE':'SALE SUMMARY - UNPAID';
  d.feed(1).bold(true).ln(title+(reprint?' - REPRINT':'')).bold(false).align('left');
  ...
  /* Compact line items: quantity + name + line total share one line when they fit. */
  for (const i of consolidated) {
    d.bold(true).itemRow(`${i.qty} x ${i.name}`, money(i.price * i.qty)).bold(false);
  ...
  d.hr();
  if (t.discount) d.row('Subtotal', money(t.subtotal));
  ...
  if ((paid || partial) && o.payments && o.payments.length) {
    d.hr();
    ...
    if (partial) d.bold(true).row('BALANCE REMAINING', money(o.balance)).bold(false);
  }
  ...
  d.feed(Math.max(0,Math.min(8,Number(s.receipt_footer_lines)||3)));
...
/* ----------------------------- return receipt ----------------------------- */
function buildReturnReceipt(ret, settings) {
  const d=new Doc(Number(settings.printer_chars)||42);
  d.align('center').bold(true).big(true).ln(settings.business_name).big(false).bold(false);
  d.feed(1).bold(true).ln('RETURN / REFUND').bold(false).align('left').hr();
  d.row('Return #',String(ret.id));d.row('Original sale #',String(ret.order_number));
  d.row('Date',String(ret.created_at||'').slice(0,16));d.row('By',ret.created_by_name||'-');d.hr();
  for(const item of ret.items||[])d.bold(true).itemRow(`${item.qty} x ${item.item_name}`,money(item.amount)).bold(false);
  d.hr();d.bold(true).big(true).row(`REFUNDED ${settings.currency_symbol||'KSh'}`,money(ret.amount)).big(false).bold(false);
  d.row('Method',String(ret.method||'').toUpperCase());d.ln(ret.reason||'');
  d.center(ret.restocked?'RETURNED TO STOCK':'NOT RETURNED TO STOCK');d.feed(3).cut();
  return d.buffer();
}
...
module.exports = { Doc, CMD, buildReceipt, buildReturnReceipt, buildKitchenTicket, send, writeToFile };
