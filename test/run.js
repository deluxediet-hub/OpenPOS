...
(async () => {
  const suites = ['domain.js', 'packaging.js', 'structure.js', 'retail.js', 'hardening.js', 'inventory-packages.js', 'e2e.js', 'features.js', 'ui.js'];
  let code = 0;
  ...
  for (const suite of suites) {
    ...
    const setupRes = await fetch(base + '/api/setup', {
      ...
      body: JSON.stringify({
        business: { business_name: ['retail.js','hardening.js','inventory-packages.js'].includes(suite) ? 'Test Wines & Spirits' : 'Test Cafe', address: '1 Test Way',
          phone: '+254700000000', kra_pin: 'P000000000T', ...(['retail.js','hardening.js','inventory-packages.js'].includes(suite) ? { business_type: 'wines_spirits' } : {}) },
